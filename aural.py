...
DEFAULT_CONFIG_FILE = "aural.cnf.SAMPLE"
...
def init(configpath):
    ...
    # create a Aural instance
    return Aural(config)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Aural: Audio Fingerprinting library",
    ...
    atr = init(config_file)
    if args.fingerprint:
        # Fingerprint all files in a directory
        if len(args.fingerprint) == 2:
            directory = args.fingerprint[0]
            extension = args.fingerprint[1]
            print(f"Fingerprinting all .{
                  extension} files in the {directory} directory")
            atr.fingerprint_directory(directory, ["." + extension], 4)
        ...
        if source in ('mic', 'microphone'):
            songs = atr.recognize(MicrophoneRecognizer, seconds=opt_arg)
        elif source == 'file':
            songs = atr.recognize(FileRecognizer, opt_arg)
        print(songs)
