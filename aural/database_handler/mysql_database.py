...
from aural.base_classes.common_database import CommonDatabase
from aural.config.settings import (FIELD_FILE_SHA1, FIELD_FINGERPRINTED,
...
class MySQLDatabase(CommonDatabase):
    ...
    SELECT_ALL = f"SELECT `{FIELD_SONG_ID}`, `{
        FIELD_OFFSET}` FROM `{FINGERPRINTS_TABLENAME}`;"
    ...
    SELECT_NUM_FINGERPRINTS = f"SELECT COUNT(*) AS n FROM `{
