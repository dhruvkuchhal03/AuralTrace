...
from aural.base_classes.base_database import get_database
from aural.config.settings import (DEFAULT_FS, DEFAULT_OVERLAP_RATIO,
...
from aural.logic.fingerprint import fingerprint
...
class Aural:
    ...
    def fingerprint_directory(self, path: str, extensions: str, nprocesses: int = None) -> None:
        ...
        # Send off our tasks
        iterator = pool.imap_unordered(Aural._fingerprint_worker, worker_input)
    ...
    def fingerprint_file(self, file_path: str, song_name: str = None) -> None:
        ...
        if song_hash in self.songhashes_set:
            ...
            song_name, hashes, file_hash = Aural._fingerprint_worker(
                file_path,
                self.limit,
                song_name=song_name
            )
    ...
    def recognize(self, recognizer, *options, **kwoptions) -> Dict[str, any]:
        r = recognizer(self)
        # print("it is here hahahahahahhhh     ")
        # print(r)
    ...
    def _fingerprint_worker(arguments):
        ...
        fingerprints, file_hash = Aural.get_file_fingerprints(
    ...
    def get_file_fingerprints(file_name: str, limit: int, print_output: bool = False):
        ...
        for channeln, channel in enumerate(channels, start=1):
            if print_output:
                print(f"Fingerprinting channel {
                      channeln}/{channel_amount} for {file_name}")
