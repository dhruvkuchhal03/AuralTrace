...
from aural.config.settings import (CONNECTIVITY_MASK, DEFAULT_AMP_MIN,
...
def fingerprint(channel_samples: List[int],
    ...
    # print("\nInside fingerprint function in logic folder\n arr2D: \n")
    # print(arr2D)
    # print("\n\n")
...
def get_2D_peaks(arr2D: np.array, plot: bool = False, amp_min: int = DEFAULT_AMP_MIN)\
    ...
    # Applying erosion, the aural documentation does not talk about this step.
    background = (arr2D == 0)
        ...
        for j in range(1, fan_value):
            if (i + j) < len(peaks):
                ...
                if MIN_HASH_TIME_DELTA <= t_delta <= MAX_HASH_TIME_DELTA:
                    h = hashlib.sha1(f"{str(freq1)}|{str(freq2)}|{
